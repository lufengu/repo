...
function NuevoPedidoModal({ open, onClose, onCreate }) {
  ...
  const handleSubmit = (e) => {
    ...
    setError('');
    onCreate({ producto, proveedor, cantidad, fecha, estado: 'Pendiente' });
    setProducto('');
    setProveedor('');
    setCantidad('');
    setFecha('');
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.3)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#fff', borderRadius: '16px', padding: '2rem', minWidth: '350px', maxWidth: '400px', width: '100%', minHeight: '350px', boxShadow: '0 4px 24px #0003', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <h3 style={{ fontWeight: 'bold', fontSize: '1.5rem', marginBottom: '1.2rem', textAlign: 'center' }}>Nuevo Pedido</h3>
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1.5rem' }}>
            ...
            <button type="submit" style={{ background: '#ff6600', color: '#fff', border: 'none', borderRadius: '8px', padding: '0.8rem 2rem', fontWeight: 'bold', fontSize: '1rem' }}>Crear</button>
          </div>
        </form>
      </div>
    </div>
